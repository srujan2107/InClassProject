...
namespace MicroMVVM.Expence.Daily
{
    public class DailyExpence
    {
        ...
        {
            List<DailyExpence> listDE = new List<DailyExpence>();
            ...
            foreach (var varDE in custs)
            {
                DailyExpence dailyExpence = new DailyExpence
                {
                    ExpenceId = varDE.Element("expenceid").Value,

                    ExpenceDate = varDE.Element("ExpenceDate").Value,

                    ExpenceFor = varDE.Element("ExpenceFor").Value,

                    AmountSpend = Convert.ToDecimal(varDE.Element("AmountSpend").Value),

                    SpendBy = varDE.Element("SpendBy").Value
                };

                listDE.Add(dailyExpence);
            }

            return listDE;
        }

        public static List<DailyExpence> IndividualSort(string loginId, string transactionType)
        {
            List<DailyExpence> listDE = new List<DailyExpence>();
        ...
        public static List<DailyExpence> prioritizedView(string loginId)
        {
            List<DailyExpence> listDE = new List<DailyExpence>();
            string appPath = System.IO.Path.GetDirectoryName(System.Reflection.Assembly.GetExecutingAssembly().CodeBase);
            XDocument document = XDocument.Load(appPath + @"\DailyExpence.xml");
        ...
        public static List<DailyExpence> showColumnChart(string transactionType)
        {
            ...
            List<DailyExpence> listDE = new List<DailyExpence>(from c in document.Descendants("expence")
